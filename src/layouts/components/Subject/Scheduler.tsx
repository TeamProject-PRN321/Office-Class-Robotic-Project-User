...
import { ClockOutline, MenuLeftOutline, MenuRightOutline } from 'mdi-material-ui'
...
const Slot = ({ data }: { data: ScheduleModel }) => {
  ...
  // const [openReportGrade, setOpenReportGrade] = useState(false)
  ...
  const handleClose = () => {
    setOpen(false)
    route.push('/student-reports/' + classModel?.studentId)
  }

  const axios = useAxios()
  const auth = useAuth()

  const handleGetClassModel = async () => {
    try {
      const classData = await axios.call(
        'get',
        'https://localhost:7254/api/v1/classes/get-class-of-student-by-id-and-classname/' +
          auth.Id +
          '/' +
          data.className
      )
  ...
  return (
    <Fragment>
      <Tooltip title={'Class: ' + data.className}>
        <Card
          ...
          sx={{
            ...
            top: (moment(data.timeStart, 'HH:mm:ss').get('minute') / 60) * heightOfASlot + 'px',
            left: '0.5px',
            right: '0.5px',
      ...
      >
        ...
        {/* <DialogContent>
          ...
          <TextField required id='outlined-required' defaultValue='PRN231' />
        </DialogContent> */}

        <Box sx={{ margin: '15px', display: 'flex', flexDirection: 'column', gap: 3 }}>
          <TextField
          ...
          <TextField
            ...
            value={data.slotAttendanceStatus}
            InputProps={{
              readOnly: true
            }}
            variant='filled'
